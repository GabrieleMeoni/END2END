...
def main():
    ...
    for event in data_loaded:
        if (data_loaded[event][0] == args.satellite) and (detector_number_str in list(data_loaded[event][1].keys())):
            event_compatible=data_loaded[event]
            break

    if event_compatible is None:
        raise ValueError(f"Impossible to find an event having satellite: "+colored(args.satellite, "red")+" and detector number: "+colored(detector_number_str)+".")
        
    
    # Granules numbers and lists
    granules_numbers_list=[list(event_compatible[1][detector_number_str].keys())[::-1] if not(int(args.detector_number) % 2) else list(event_compatible[1][detector_number_str].keys())][0]
    ...
    onboard_processor=oboardDetectorProcessor(args.satellite, args.detector_number, model_name=args.model, depth=args.depth, widen_factor=args.widen_factor, leaky_slope=args.leaky_slope, dropout=args.dropout)
    ...
    onnx_path=os.path.join(OUTPUT_DIR, "onboard_processor_D"+detector_number_str+ "_"+args.model+".onnx")
