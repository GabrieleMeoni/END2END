...
def main():
    ...
    print(
        "Load model: "
        + colored(args.model, "green")
        + " from checkpoint: "
        + colored(checkpoint_path, "blue")
    )
    ...
    net_eval = _net_builder(
        num_classes=_test_dset.num_classes, in_channels=_test_dset.num_channels
    )
    ...
    dummy_input = torch.randn(
        1, _test_dset.num_channels, _test_dset.size[0], _test_dset.size[1]
    )
