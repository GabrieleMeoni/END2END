...
def geographical_splitter(
    ...
    volcanoes_events_keys_shuffled = deepcopy(
        list(volcano_events_locations_dict.keys())
    )
    fires_events_keys_shuffled = deepcopy(list(fire_events_locations_dict.keys()))
    volcanoes_not_events_keys_shuffled = deepcopy(
        list(volcano_not_events_locations_dict.keys())
    )
    fires_not_events_keys_shuffled = deepcopy(
        list(fire_not_events_locations_dict.keys())
    )
    ...
    n_events = np.sum(
        np.array(
            [n for n in fire_events_locations_dict.values()]
            + [n for n in volcano_events_locations_dict.values()]
        )
    )
    # Number of total nonevents
    n_not_events = np.sum(
        np.array(
            [n for n in fire_not_events_locations_dict.values()]
            + [n for n in volcano_not_events_locations_dict.values()]
        )
    )
    ...
    while n_events_test < n_events_test_max:
        n_events_test += volcano_events_locations_dict[
            volcanoes_events_keys_shuffled[volcano_label_idx]
        ]
        n_events_list.append(
            volcano_events_locations_dict[
                volcanoes_events_keys_shuffled[volcano_label_idx]
            ]
        )
        events_locations_tests_list.append(
            volcanoes_events_keys_shuffled[volcano_label_idx]
        )
        volcano_label_idx += 1

        if n_events_test < n_events_test_max:
            n_events_test += fire_events_locations_dict[
                fires_events_keys_shuffled[fire_label_idx]
            ]
            n_events_list.append(
                fire_events_locations_dict[fires_events_keys_shuffled[fire_label_idx]]
            )
            events_locations_tests_list.append(
                fires_events_keys_shuffled[fire_label_idx]
            )
    ...
    while n_not_events_test < n_not_events_test_max:
        n_not_events_test += volcano_not_events_locations_dict[
            volcanoes_not_events_keys_shuffled[volcano_label_idx]
        ]
        n_non_events_list.append(
            volcano_not_events_locations_dict[
                volcanoes_not_events_keys_shuffled[volcano_label_idx]
            ]
        )
        not_events_locations_tests_list.append(
            volcanoes_not_events_keys_shuffled[volcano_label_idx]
        )
        volcano_label_idx += 1

        if n_not_events_test < n_not_events_test_max:
            n_not_events_test += fire_not_events_locations_dict[
                fires_not_events_keys_shuffled[fire_label_idx]
            ]
            not_events_locations_tests_list.append(
                fires_not_events_keys_shuffled[fire_label_idx]
            )
    ...
    if (
        abs(real_percentage - test_size_percentage)
        < split_percentage_error_tolerance * real_percentage
    ):
        raise ValueError(
    ...
    X_train = np.zeros(
        [
            len(labels) - (n_events_test + n_not_events_test),
            images[0].shape[0],
            images[0].shape[1],
            images[0].shape[2],
        ],
    ...
    y_train = np.zeros(
        [len(labels) - (n_events_test + n_not_events_test)], dtype="int64"
    )
    # Images and labels for tests
    X_test = np.zeros(
        [
            n_events_test + n_not_events_test,
            images[0].shape[0],
            images[0].shape[1],
            images[0].shape[2],
        ],
    ...
    for n, filename in enumerate(filenames):
        ...
        if ((location in events_locations_tests_list) and labels[n] == 0) or (
            (location in not_events_locations_tests_list) and (labels[n] == 1)
        ):
...
class THRAWS_train_dataset(torch.utils.data.Dataset):
    ...
    def _load_data(self):
        ...
        X_train, X_test, y_train, y_test = geographical_splitter(
            images, labels, filenames_sorted, self.test_ratio, self.seed
        )

        # Add upsampling.
        if self.upsample_ratio is not None:
            NE, EV = self.upsample_ratio
            X_train, y_train = upsample_ds(
                ds=X_train, lb=y_train, NotEvent=NE, Event=EV
            )
